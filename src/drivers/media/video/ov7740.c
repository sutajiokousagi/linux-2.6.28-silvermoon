...
static int ov7740_detect(struct i2c_client *client)
{
	...
	if (ret < 0)
		return ret;
	if (v != 0x42)//ov spec err, should be 0x42 not 0x40
		return -ENODEV;
	return 0;
}
...
static struct ov7740_format_struct {
	...
	{
		.desc		= "OV7740 VGA YUV422 60fps",
		.pixelformat	= V4L2_PIX_FMT_YUV422P,
		.width		= VGA_WIDTH,
		.height		= VGA_HEIGHT,
		.fps		= 60,
		.regs 		= OV7740_VGA_YCbCr_60fps,
		.bpp		= 16,
	},
	{
		...
		void *arg)
{
	...
	switch (cmd) {
		case VIDIOC_G_CHIP_IDENT:
			return v4l2_chip_ident_i2c_client(client, arg, V4L2_IDENT_OV7740, 0);
