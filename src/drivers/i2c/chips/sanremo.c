...
static void sanremo_start_charging(void)
{
	...
	if(!(tmp & SANREMO_CHG_STATUS)) {
		...
		val += tmp<<4;
		voltage = (unsigned int)(val * 13185 / 10000);
	}

	if(voltage < 3800) {
		printk(KERN_INFO "Voltage is low, start charging\n");
		...
		sanremo_write(SANREMO_CHG_CTRL1, 0xfc); 
		/*set current limit at 300 mA*/
		sanremo_write(SANREMO_CHG_CTRL2, 0x05); 
		...
		/*enable fast charing*/
		sanremo_write(SANREMO_CHG_CTRL1, 0xfe); 
...
static void sanremo_charger_init(void)
{
	...
	sanremo_write(SANREMO_INTERRUPT_ENABLE1, 
			tmp|SANREMO_CHG_MASK|SANREMO_BAT_MASK);
...
static void sanremo_turn_off_power(void)
{
	u8 tmp;
...
static int get_power_supply_module(int cmd)
{
	...
	command = sanremo_power_module[cmd - VCC_CORE].command;
	if (command != cmd) {
		printk(KERN_WARNING "micco_power_module[] is error: %d\n", cmd);
		return -EINVAL;
	}
	power_module = sanremo_power_module[cmd - VCC_CORE].power_module;
	if (power_module == 0) {
			...
			const struct i2c_device_id *id)
{
	...
	pdev = platform_device_alloc("sanremo_touch", (int)id->driver_data);
