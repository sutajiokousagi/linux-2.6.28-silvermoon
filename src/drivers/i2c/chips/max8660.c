...
static int set_max8660_voltage(int cmd, int mv)
{
	unsigned char val;
	int status = 0;
...
static int get_max8660_voltage(int cmd, int *pmv)
{
	unsigned char val;
	int status = 0;
	int power_module = get_power_supply_module(cmd);
...
#ifdef	CONFIG_PM
static int vcc_misc1, vcc_misc2, hdmi_1p2v;
...
postcore_initcall(max8660_init);
