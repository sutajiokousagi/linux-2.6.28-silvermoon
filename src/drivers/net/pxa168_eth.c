...
static irqreturn_t pxa168_eth_int_handler(int irq, void *dev_id)
{
	...
	if (icr & (ICR_RXBUF | ICR_RXERR | ICR_TXBUF_H |
		...
		if (netif_rx_schedule_prep(dev, &mp->napi)) {
			/* Disable interrupts */
			wrl(mp, INT_MASK, 0);
			__netif_rx_schedule(dev, &mp->napi);
		}
	}
...
static int pxa168_eth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	} else if (mp->tx_ring_size - mp->tx_desc_count < MAX_DESCS_PER_HIGH) {
		netif_stop_queue(dev);
		return NETDEV_TX_BUSY;
	}
...
static int pxa168_eth_probe(struct platform_device *pdev)
{
	...
	mustUsePolling = cpu_is_pxa168_A0();

	if (mustUsePolling)
	{
		printk (KERN_NOTICE "pxa168: polling mode enabled\n");
	}
