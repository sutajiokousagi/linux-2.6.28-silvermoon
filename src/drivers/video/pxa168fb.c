...
#define CHLOG(format, arg...)            \
	printk("pxa168fb.c - %s():%d - " format, __func__, __LINE__, ## arg)
...
static int pxa168_graphic_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
        ...
        case FBIOPUT_VIDEO_ALPHABLEND:
                ...
                val |= CFG_ALPHA(0xff);
				CHLOG("Setting alpha blend to something static\n");
...
static void pxa168fb_set_default(struct pxa168fb_info *fbi, struct pxa168fb_mach_info *mi)
{
	 ...
	 * Dumb panel vsync triggers DMA [30:28]
	 * Rising edge triggers DMA [27]
	 * Color key mode disabled [26:24]
	 * Zero fill low bits for RGB565 -> 24 bit [23]
	 * Line buffer disabled [22]
	 * Gated clock disabled [21]
	 * * Power save disabled [20]
	 * No smart panel scaling [19:18]
	 * Configurable alpha [17:16]
	 * Configure alpha all overlay plane [15:8]
	 * Reserved pixel command value [7:0]
	 */
...
static int __init pxa168fb_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_MACH_CHUMBY_SILVERMOON)
	/* 
	 * Reset LCD parameters, to elimitate a black flicker on boot.
	 * The other framebuffer ought to be all set up already so hide this
	 * one.
	 */
	CHLOG("Changing LCD_SPU_DMA_CTRL1 from 0x%08x -> 0x2002ff81\n", readl(fbi->reg_base + LCD_SPU_DMA_CTRL1));
	 ...
	 */
	CHLOG("Changing LCD_SPU_DMA_CTRL0 from 0x%08x -> 0x08441111\n", readl(fbi->reg_base + LCD_SPU_DMA_CTRL0));
