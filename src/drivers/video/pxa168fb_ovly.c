			...
			unsigned long arg)
{
	...
	case FB_IOCTL_SWITCH_VID_OVLY:
		if (copy_from_user(&vid_on, argp, sizeof(int)))
			return -EFAULT;
#ifndef CONFIG_MACH_CHUMBY_SILVERMOON
		/* We always leave this on for Silvermoon */
		if (0 == vid_on) {
...
static void set_dma_control0(struct pxa168fb_info *fbi)
{
	...
	*/
	if ((fbi->pix_fmt & 0x1000) || (fbi->pix_fmt == 10) || (fbi->pix_fmt == 11)) {
                x |= 0x00000002;
...
static void set_graphics_start(struct fb_info *fi, int xoffset, int yoffset)
{
	...
	if (!(fbi->new_addr[0])) {
		...
		dma_base_address = fbi->fb_start_dma + ((pixel_offset * var->bits_per_pixel) >> 3);
	}
	
	/* Android system does not want us to synchronize the dma address to VSync */
	if(is_android()) {
		if (fbi->new_addr[0]) {
...
static int pxa168fb_set_par(struct fb_info *fi)
{
	...
	/* make sure that there is a sensible initial value */
	if (fbi->surface.viewPortInfo.yPitch == 0) {
		fbi->surface.viewPortInfo.yPitch = var->xres_virtual;
	}

	if (fbi->surface.viewPortInfo.uPitch == 0) {
		if (((fbi->pix_fmt & ~0x1000) >> 1) == 5) {
			fbi->surface.viewPortInfo.uPitch = var->xres_virtual;
		} else {
			fbi->surface.viewPortInfo.uPitch = var->xres_virtual >> 1;
		}
	}

	if (fbi->surface.viewPortInfo.vPitch == 0) {
		if (((fbi->pix_fmt & ~0x1000) >> 1) == 5) {
			fbi->surface.viewPortInfo.vPitch = var->xres_virtual;
		} else {
			fbi->surface.viewPortInfo.vPitch = var->xres_virtual >> 1;
		}
	}
	...
	if ((fbi->pix_fmt >= 0) && (fbi->pix_fmt < 10)) {
		...
		if (((fbi->pix_fmt & ~0x1000) >> 1) == 5)
			...
			writel(fbi->surface.viewPortInfo.yPitch,
			       fbi->reg_base + LCD_SPU_DMA_PITCH_YC);
			writel((fbi->surface.viewPortInfo.vPitch << 16) |
...
static struct fb_ops pxa168fb_ops = {
	...
	/* imgtool goes reeeealy slow with sync, because it uses write() */
#ifndef CONFIG_MACH_CHUMBY_SILVERMOON
