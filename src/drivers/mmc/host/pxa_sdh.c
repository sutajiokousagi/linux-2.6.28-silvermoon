...
static int pxa_sdh_probe(struct platform_device *pdev)
{
	...
	/* set 4 bits bus width default */
	mmc->caps = MMC_CAP_SDIO_IRQ | MMC_CAP_MMC_HIGHSPEED |
				MMC_CAP_SD_HIGHSPEED | MMC_CAP_4_BIT_DATA;
	if (host->pdata->bus_width) {
		if (host->pdata->bus_width == 8) /* board supports 8 bits bus */
			mmc->caps |= MMC_CAP_4_BIT_DATA | MMC_CAP_8_BIT_DATA;
		if (host->pdata->bus_width == 1) /* board supports 1 bit bus */
