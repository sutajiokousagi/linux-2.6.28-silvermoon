...
extern void mv_usb_connect_change(int status);
/*----------------------------------------------------------------*/
...
static int pxa_otgc_interrupt_handle(struct pxa_otg *pOtgHandle)
{
	...
	if (machine_is_teton_bga())
		if ((tmp & U2xOTGSC_BSVIS) && mini_a_plugin)
			return OTG_INT_INIT;
		...
		if (u2o_get(u2otg->regbase, U2xOTGSC) & U2xOTGSC_ID) {
			if (machine_is_teton_bga()) {
				if (mini_a_plugin) {
					int_type |= OTG_INT_IDR;
					mini_a_plugin = 0;
					u2o_clear(u2otg->regbase, U2xPORTSC, U2xPORTSC_PP);
				} else {
					if (u2o_get(u2otg->regbase, U2xOTGSC) & U2xOTGSC_BSV)
						mv_usb_connect_change(USBOTG_VBUS_VALID);
					if (u2o_get(u2otg->regbase, U2xOTGSC) & U2xOTGSC_BSE)
