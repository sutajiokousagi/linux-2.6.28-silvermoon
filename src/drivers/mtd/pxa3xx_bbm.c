...
static int sync_pxa3xx_bbt(struct mtd_info *mtd, loff_t ofs)
{
	...
	while (1) {
		...
		instr.mtd = mtd;
		instr.addr = reloc_block << mtd->erasesize_shift;
...
int pxa3xx_update_bbt(struct mtd_info *mtd, loff_t offs)
{
	...
	do {
		...
		case BBM_NEW:
			new_bbm = (struct pxa3xx_new_bbm *)bbm->data_buf;
			part = find_part(mtd, offs);
			offset = (new_bbm->rbbt_offset[part] + 1)
				<< mtd->writesize_shift;
			if (!offset & mtd->erasesize_mask)
				goto ERR_EXIT;
...
int pxa3xx_block_bad(struct mtd_info *mtd, loff_t ofs, int allowbbt)
{
	...
	if (bbm && (bbm->is_init != BBT_NOINIT)) {
		...
		case BBM_NEW:
			new_bbm = (struct pxa3xx_new_bbm *)bbm->data_buf;
			if (ofs < (BOOT_PRAT_MAX << mtd->erasesize_shift))
				return boot_part_bad(mtd, ofs);
			else {
				part = find_part(mtd, ofs);
...
static int recover_new_bbm(struct mtd_info *mtd)
{
	...
	int block, total_block, reserved_block;
	int rbbt, rbbt_back, max_reloc_entry;
	loff_t ofs;
	size_t retlen;
	...
	if (new_bbm->main_block == -1 && new_bbm->back_block == -1) {
		...
		for (block = 0; block < BOOT_PRAT_MAX; block ++) {
			instr.addr = block << mtd->erasesize_shift;
			should_reloc = 0;
			mtd->erase(mtd, &instr);
			should_reloc = 1;
			if (!erase_success)
		...
		printk(KERN_INFO "Get main block at %d, back block at %d\n",
				new_bbm->main_block, new_bbm->back_block);
		...
		new_bbm->partinfo->start_addr = block << mtd->erasesize_shift;
		new_bbm->partinfo->end_addr =
		  ((total_block - reserved_block) << mtd->erasesize_shift) - 1;
		new_bbm->partinfo->rp_start =
			(total_block - reserved_block) << mtd->erasesize_shift;
		new_bbm->partinfo->rp_size = reserved_block << mtd->erasesize_shift;
		new_bbm->partinfo->rp_algo = RP_UPWD;
		new_bbm->partinfo->rbbt_type = PXA_NEW_BBM_HEADER;
		new_bbm->partinfo->rbbt_start = rbbt << mtd->erasesize_shift;
		new_bbm->partinfo->rbbt_start_back =
					rbbt_back << mtd->erasesize_shift;

		max_reloc_entry = (mtd->writesize - 40)
			/ sizeof(struct reloc_item);
		...
		/* perform fbbt sync task and build pt */
		for (block = BOOT_PRAT_MAX, ofs = block * mtd->erasesize;
			ofs < mtd->size; ofs += mtd->erasesize, block ++) {
			printk(KERN_INFO "gather run time bbt info"
					" at %16llx..\r", ofs);
			if (checkbad(mtd, ofs)) {
				printk(KERN_INFO "\nmark %llx as bad in fbbt\n",
						ofs);
				sync_pxa3xx_bbt(mtd, ofs);
				update_fbbt(fbbt, block);
			}
		...
		ofs = (PXA_BEGIN_SLOT << mtd->writesize_shift)
			+ (new_bbm->main_block << mtd->erasesize_shift);

		printk(KERN_INFO "\nBegin to write main block..\n");
		mtd->write(mtd, ofs, mtd->writesize, &retlen, (void *)fbbt);
		ofs = ((PXA_BEGIN_SLOT + 1) << mtd->writesize_shift)
			+ (new_bbm->main_block << mtd->erasesize_shift);
		mtd->write(mtd, ofs, mtd->writesize, &retlen, (void *)part);
		ofs = rbbt << mtd->erasesize_shift;
		mtd->write(mtd, ofs, mtd->writesize, &retlen,
					(void *)new_bbm->rbbt);
		ofs = rbbt_back << mtd->erasesize_shift;
		mtd->write(mtd, ofs, mtd->writesize, &retlen,
					(void *)new_bbm->rbbt);

		if (new_bbm->back_block == -1) {
			printk(KERN_WARNING "There is only one good block!\n");
			printk(KERN_WARNING "You'd better replace this!!\n");
		}
		else {
			printk(KERN_INFO "Begin to write back block..\n");
			ofs = (PXA_BEGIN_SLOT << mtd->writesize_shift)
				+ (new_bbm->back_block << mtd->erasesize_shift);
			mtd->write(mtd, ofs, mtd->writesize, &retlen,
						(void *)fbbt);
			ofs = ((PXA_BEGIN_SLOT + 1) << mtd->writesize_shift)
				+ (new_bbm->back_block << mtd->erasesize_shift);
		...
		printk(KERN_INFO "done!!\n");
		new_bbm->main_block = new_bbm->back_block = -1;
		return 0;
	}
	...
	if (new_bbm->back_block == -1) {
		...
		for (block = 0; block < BOOT_PRAT_MAX; block ++,
			...
			if (block == new_bbm->main_block
					|| pxa3xx_block_bad(mtd, instr.addr, 0))
				continue;
		...
		if (new_bbm->back_block == -1) {
			printk(KERN_WARNING "Unable to back up boot blcok!!\n");
		}
	}

	/*
	 * The main block and back block is still there
	 * only do refresh task here
	 * first refresh the back block, then come to the main block
	 */
	return 0;
}
