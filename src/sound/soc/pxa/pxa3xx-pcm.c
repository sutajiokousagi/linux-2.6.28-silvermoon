...
static int pxa3xx_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	if (cmd != prev_cmd /*|| call_count++ % 16 == 0*/)
	{
		printk( KERN_INFO "%s:%d %s(...,0x%lx, %u) prtd=0x%08lx\n",
			__FILE__, __LINE__, __FUNCTION__,
			(unsigned long)substream, cmd, (unsigned long)prtd );
	}
	prev_cmd = cmd;
#endif
	switch (cmd) {
...
static int pxa3xx_pcm_close(struct snd_pcm_substream *substream)
{
	...
	struct pxa3xx_runtime_data *prtd = runtime->private_data;
#ifdef CONFIG_SND_PXA3XX_SOC_SILVERMOON_DEBUG
	printk( KERN_INFO "pxa3xx-pcm.c:%d %s(0x%08lx) dev=0x%08lx\n", __LINE__, __FUNCTION__, (unsigned long)substream, (unsigned long)substream->pcm->card->dev );
#endif
...
#ifdef CONFIG_SND_PXA3XX_SOC_SILVERMOON_DEBUG
	printk( KERN_INFO "pxa3xx-pcm.c:%d %s(0x%08lx,0x%08lx) dev 0x%08lx area 0x%08lx dma_addr 0x%08lx len=%lu\n",
		__LINE__, __FUNCTION__, (unsigned long)substream, (unsigned long)vma,
		(unsigned long)substream->pcm->card->dev,
		runtime->dma_area, runtime->dma_addr, (unsigned long)runtime->dma_bytes
		 );
#endif
