...
#define HEADPHONE_GPIO 97
#define HEADPHONE_IRQ IRQ_GPIO(HEADPHONE_GPIO)
#endif
...
static struct snd_soc_device *wm8961_socdev;
static struct device *wm8961_codec_dev = NULL;
...
#ifdef CHUMBY_HEADPHONE_DETECT
static int work_queued = 0;
static struct delayed_work event_work;
static struct snd_soc_codec *work_codec;

static inline int read_gpio_line(int gpio)
{
	// read the hp insertion interrupt line: bit off means hp is inserted
	return !((__raw_readl(GPIO_REGS_VIRT+0x100)) & GPIO_bit(gpio));
}


static void headphone_update(struct work_struct *work) {
	struct snd_soc_codec *codec = work_codec;
	char *message;
	char *envp[2];

	u16 spk_reg = wm8961_read(codec, WM8961_CLASS_D_CONTROL_1);

	if (read_gpio_line(HEADPHONE_GPIO)) {
		CHLOG("Headphone inserted\n");
		message = "HEADPHONES=1";
		/* Disable the amplifier */
		spk_reg &= ~(WM8961_SPKL_ENA | WM8961_SPKR_ENA);
	}
	else {
		CHLOG("Headphone removed\n");
		message = "HEADPHONES=0";
    ...
    envp[0] = message;
    envp[1] = NULL;
    kobject_uevent_env(&wm8961_codec_dev->kobj, KOBJ_CHANGE, envp);
...
static irqreturn_t headphone_detect_isr(int irq, void *dev_id)
{
    /* Debounce the port by queueing the handler to happen in 2 msecs */
    int msecs = 5;
    if(!work_queued) {
        work_queued = 1;
		work_codec = &((struct wm8961_priv *)dev_id)->codec;
		INIT_DELAYED_WORK(&event_work, headphone_update);
		schedule_delayed_work(&event_work, msecs);
    }
	return IRQ_HANDLED;
}

#endif // CHUMBY_HEADPHONE_DETECT



static int wm8961_run_sequence(struct snd_soc_codec *codec, int start,
			       int timeout)
{
	u16 reg;

	wm8961_write(codec, WM8961_WRITE_SEQUENCER_1, WM8961_WSEQ_ENA);
	wm8961_write(codec, WM8961_WRITE_SEQUENCER_4,
		     WM8961_WSEQ_START | start);
	do {
		reg = wm8961_read(codec, WM8961_WRITE_SEQUENCER_7);
		msleep(1);
	} while (reg & WM8961_WSEQ_BUSY && --timeout);

	wm8961_write(codec, WM8961_WRITE_SEQUENCER_1, 0);

	if (!timeout) {
		dev_err(wm8961_codec_dev, "Timed out running sequence\n");
		return -EIO;
	}
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	if (event & SND_SOC_DAPM_PRE_PMU) {
		wm8961->charge_pump_users++;

		dev_dbg(wm8961_codec_dev, "Charge pump use count now %d\n",
			wm8961->charge_pump_users);

		if (wm8961->charge_pump_users == 1) {
			dev_dbg(wm8961_codec_dev, "Enabling charge pump\n");
	...
	if (event & SND_SOC_DAPM_POST_PMD) {
		...
		if (wm8961->charge_pump_users == 0) {
			dev_dbg(wm8961_codec_dev, "Disabling charge pump\n");
...
#ifdef CHUMBY_HEADPHONE_DETECT
	// Kick the headphone detector.
	work_codec = codec;
	if(!work_queued)
		headphone_update(NULL);
#endif
...
static int wm8961_add_controls(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < ARRAY_SIZE(wm8961_snd_controls); i++) {
		err = snd_ctl_add(codec->card,
				  snd_soc_cnew(&wm8961_snd_controls[i],
					       codec, NULL));
		if (err < 0)
			return err;
	}
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	if (!wm8961->sysclk) {
		dev_err(wm8961_codec_dev, "MCLK has not been specified\n");
		return -EINVAL;
	}
	...
	dev_dbg(wm8961_codec_dev, "Selected SRATE %dHz for %dHz\n",
	...
	if (i == ARRAY_SIZE(wm8961_clk_sys_ratio)) {
		dev_err(wm8961_codec_dev, "Unable to generate CLK_SYS_RATE\n");
		return -EINVAL;
	}
	dev_dbg(wm8961_codec_dev, "Selected CLK_SYS_RATE of %d for %d/%d=%d\n",
			     ...
			     int dir)
{
	...
	if (freq > 33000000) {
		dev_err(wm8961_codec_dev, "MCLK must be <33MHz\n");
		return -EINVAL;
	}

	if (freq > 16500000) {
		dev_dbg(wm8961_codec_dev, "Using MCLK/2 for %dHz MCLK\n", freq);
		reg |= WM8961_MCLKDIV;
		freq /= 2;
	} else {
		dev_dbg(wm8961_codec_dev, "Using MCLK/1 for %dHz MCLK\n", freq);
		reg &= WM8961_MCLKDIV;
	}
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		if (codec->bias_level == SND_SOC_BIAS_STANDBY) {
			/* Clock the digital parts */
			reg = wm8961_read(codec, WM8961_CLOCKING2);
			reg |= WM8961_CLK_DSP_ENA;
			wm8961_write(codec, WM8961_CLOCKING2, reg);
...
static int wm8961_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(wm8961_codec_dev, "failed to create pcms: %d\n", ret);
		goto pcm_err;
	}

	wm8961_add_controls(codec);
	...
	if (ret < 0) {
		dev_err(wm8961_codec_dev, "wm8961: failed to register card\n");
		goto card_err;
	}
...
static int wm8961_register(struct wm8961_priv *wm8961)
{
	...
	if (wm8961_codec) {
		dev_err(wm8961_codec_dev, "Another WM8961 is registered\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (reg != 0x1801) {
		dev_err(wm8961_codec_dev, "Device is not a WM8961: ID=0x%x\n", reg);
		ret = -EINVAL;
		goto err;
	}

	reg = wm8961_read_hw(codec, WM8961_RIGHT_INPUT_VOLUME);
	dev_info(wm8961_codec_dev, "WM8961 family %d revision %c\n",
	...
	if (ret < 0) {
		dev_err(wm8961_codec_dev, "Failed to issue reset\n");
		return ret;
	}
...
#if defined(CONFIG_MACH_CHUMBY_SILVERMOON)
	reg = wm8961_read(codec, WM8961_LEFT_INPUT_VOLUME);
	wm8961_write(codec, WM8961_LEFT_INPUT_VOLUME, reg | WM8961_IPVU);
#endif
...
#ifdef CHUMBY_HEADPHONE_DETECT
	/* set up headphone detect IRQ */
	set_irq_type(HEADPHONE_IRQ, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
	if (request_irq(HEADPHONE_IRQ, headphone_detect_isr, IRQF_DISABLED,
						                    "headphone-detect", wm8961)) {
		dev_err(wm8961_codec_dev, "failed to request headphone-detect interrupt\n");
		goto card_err;
	}
...
card_err:
	snd_soc_free_pcms(wm8961_socdev);
	snd_soc_dapm_free(wm8961_socdev);
				      ...
				      const struct i2c_device_id *id)
{
	...
	struct snd_soc_codec *codec;
	int ret;
	struct snd_soc_device *socdev = wm8961_socdev;
	...
	wm8961_codec_dev = &i2c->dev;

	ret = wm8961_register(wm8961);
