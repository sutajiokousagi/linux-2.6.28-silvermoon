...
/*
0xd1200000 -> PIO base address

Memory READ steps:

1) Program the DMA PCIe control register to specify it is a memory read type
2) Program the PIO address register (one is MSP (64 bits) and the other
   LSP (32 bits))
3) After that, program the PIO start register
4) After that, if successful, I see the on the PIO READ data register
		     ...
		     u8 size)
{
	...
	if (is_conf_read)
		__raw_writel(((1 << PCIE_DMA_PCIE_CTRL_PCIETD_OFFSET) |
			      (PCIE_DMA_PCIE_CTRL_TLP_CFG_TYPE0_RW & 0x1F)),
	...
	__raw_writel((PCIE_DMA_MODE_NONCHAINED |
		PCIE_DMA_MODE_PCIE_READ), pcie_channels[channel].dma_mode_ch);
				  ...
				  u32 addr, u8 size, u32 val)
{
	...
	__raw_writel((PCIE_DMA_MODE_NONCHAINED |
		PCIE_DMA_MODE_PCIE_WRITE), pcie_channels[channel].dma_mode_ch);
...
{
	/*
	 * Enable Power to PCIE Controller Unit.
	 * FIXME - Power is enabled to PCIE
	 * Controller on Aspenite Board using Max7312 GPIO
	 * Expander. The Max7312 Driver is getting
	 * initialized after the PCI Subsystem
	 * initialization which is causing the routine
	 * below to fail. Current workaround is
	 * to enable power to controller from U-Boot.
	 */
	pxa168_gpio_pcie_init(); /* TODO: Get this from platform data instead */
	/*
	 * Magic Register settings to init PCI-E Link
	 * (based on some test scripts)
	 */
	__raw_writel(0x02000200, PCIE_REG(0x3020));
	__raw_writel(0x40012001, PCIE_REG(0x3024));
	__raw_writel(0x1, PCIE_REG(0x3028));
	__raw_writel(0x1, PCIE_REG(0x302c));
	__raw_writel((__raw_readl(PCIE_REG(0x3708)) & 0xFFFFFF00),
		     PCIE_REG(0x3708));
	__raw_writel((__raw_readl(PCIE_REG(0x3710)) | 0x00000080),
		     PCIE_REG(0x3710));
	__raw_writel((__raw_readl(PCIE_REG(0x3118)) | 0x00000040),
		     PCIE_REG(0x3118));
	__raw_writel((__raw_readl(PCIE_REG(0x3080)) | 0x00000003),
		     PCIE_REG(0x3080));
	__raw_writel((__raw_readl(PCIE_REG(0x3044)) & 0xFFFFFFFC),
		     PCIE_REG(0x3044));
	/* __raw_writel(0x1011000B, PCIE_REG(0x3044)); */
...
{
	/* TODO: Read PCIE Status 1 Reg. to determine if
	 * link is up */
...
static struct pci_bus __init *pxa168_pcie_scan_bus(int nr,
						   struct pci_sys_data
						   *sys)
{
	struct pci_bus *bus;
...
static void __init add_pcie_port(void)
{
	...
	__raw_writel(0xFF, AXI_VIRT_BASE + 0x82900);	/* Enable Clock to
							   PCIE Ctlr */
	...
	if (pxa168_pcie_link_up())
		...
		printk(KERN_ERR "PCIe: %s: Error: link down, ignoring\n",
		       __func__);
}
